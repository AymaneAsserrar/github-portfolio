...
export default function TerminalLoader({ onComplete }: TerminalLoaderProps) {
  ...
  const cursor = useMemo(
    () => (
      <motion.span
        className="inline-block w-[8px] h-[18px] bg-blue-500 ml-[2px] align-middle"
        animate={{ opacity: [1, 0] }}
  ...
  return (
    ...
    >
      ...
      {/* Terminal window */}
      <div className="relative w-[90vw] max-w-xl rounded-lg border border-blue-900/60 bg-[#0d1117] shadow-2xl shadow-blue-900/20 overflow-hidden">
        {/* Title bar */}
        <div className="flex items-center gap-2 px-4 py-2.5 bg-[#161b22] border-b border-blue-900/40">
          <span className="w-3 h-3 rounded-full bg-[#ff5f57]" />
          <span className="w-3 h-3 rounded-full bg-[#febc2e]" />
          <span className="w-3 h-3 rounded-full bg-[#28c840]" />
          <span className="ml-3 text-xs text-blue-500/70 font-mono select-none">
            portfolio@dev:~
          </span>
        </div>
        ...
        <div className="p-5 font-mono text-sm leading-relaxed min-h-[180px]">
          ...
          {visibleLines.map((line, i) => (
            <div key={i} className="flex items-center gap-2">
              ...
              <span className="text-blue-700/60 text-xs ml-auto select-none">✓</span>
            </div>
          ))}
